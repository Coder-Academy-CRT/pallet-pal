...
function LotCard({ lot }) {
    ...
    bag_sizes.push(
        <li key={bag_sizes.length}>
            <h3>{total_amount == '0' ? 'no stock' : `${total_amount} kg`}</h3>
        </li>
    )
    ...
    return editMode ? (
        <UpdateLot
            lot={lot}
            lot_stocks={lot_stocks}
            setEditMode={setEditMode}></UpdateLot>
    ) : (
        <>
            <div className='lotCard'>
                <button id='lotCardLhs' onClick={() => setEditMode(true)}>
                    edit
                </button>
