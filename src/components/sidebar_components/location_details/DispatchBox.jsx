...
export default function DispatchBox() {
    const {
        state: { selectedPallet, products },
        dispatch
    } = useContext(palletpalContext)
    const filteredProduct = products.filter(
        (product) => product.pallet_id == selectedPallet
    )
    ...
    const initialChangeOfProduct = productList.map((product) => ({
        productId: product.product_id,
        numberOfBag: product.number_of_bags
    }))
    ...
    const handleChange = (e) => {
        ...
        setFormData((prevState) => {
            return {
                ...prevState,
                changeOfProduct: [
                    ...formData.changeOfProduct,
                    {
                        productId: parseInt(productId),
                        numberOfBag: e.target.value
                    }
                ]
            }
        })
    ...
    const handleDelete = (e) => {
        ...
        if (isConfirmed) {
            setProductList((prevState) =>
                prevState.filter(
                    (product) => product.product_id != e.target.parentElement.id
                )
            )
        }
    }
            ...
            <div className='text-wrapper'>
                ...
                {productList.length > 0 ? (
                    <form onSubmit={handleSubmit}>
                        {productList.map((product) => (
                            <div
                                ...
                                <div>
                                    <p>
                                        Bag Size: {parseInt(product.bag_size)}
                                    </p>
                                </div>
                        ...
                        <div className='button-wrapper'>
                            <button style={styleButton} onClick={handleClose}>
                                Cancel
                            </button>
                            <button
                                type='submit'
                    ...
                    <div style={{ textAlign: 'center' }}>
                        <h3>
                            No product exist on this pallet, this pallet will be
                            removed.
                        </h3>
                        <button style={styleButton} onClick={handleClose}>
                            Confirm
